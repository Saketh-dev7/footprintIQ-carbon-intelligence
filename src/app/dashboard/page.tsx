...
import { Globe, TrendingUp, Sparkles, AlertCircle, CheckCircle2, Loader2, Calendar, Target, ArrowRight, Leaf, ShieldCheck } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background pb-32">
      ...
      <main className="container mx-auto px-6 py-12" role="main">
                  ...
                  <div className="space-y-6 animate-in fade-in duration-700">
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-bold text-primary">{aiInsights.monthlyReductionGoal.currentProgressPercent}%</span>
                      </div>
                      <Progress value={aiInsights.monthlyReductionGoal.currentProgressPercent} className="h-2 bg-white/5" />
                    </div>
        ...
        {aiInsights?.actionPlan && (
          <section className="space-y-8 mt-16 animate-in slide-in-from-bottom duration-700" aria-label="30-Day Plan">
            ...
            <Tabs defaultValue="week-1" className="w-full">
              <TabsList className="grid grid-cols-4 h-14 p-1 glass border-white/5 rounded-2xl mb-8">
                {weeks.map((w, idx) => (
                  <TabsTrigger key={idx} value={`week-${idx + 1}`} className="rounded-xl font-bold uppercase tracking-widest text-[10px] md:text-xs">
                    {w.name}
                  </TabsTrigger>
                ))}
              </TabsList>
              {weeks.map((w, idx) => (
                <TabsContent key={idx} value={`week-${idx + 1}`} className="outline-none">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-4">
                    {w.days.map((day) => (
                      <Card key={day.day} className="glass border-white/5 rounded-2xl hover:border-primary/30 transition-all overflow-hidden group">
...
function MetricCard({ label, value, sub, icon: Icon, color }: { label: string, value: string, sub: string, icon: any, color: string }) {
  return (
    <Card className="glass border-white/5 rounded-[2.5rem] p-6 shadow-lg hover:translate-y-[-4px] transition-transform">
